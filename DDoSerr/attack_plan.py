...
class Plan:
    ...
    #Так как они вне функций, то к ним можно обратится
    #при импорте класса!
    plan_url = []
    plan_pause = []
    ...
    def plan(self):
        ...
        for line in self.plan_file:
            #Выбираем строки со ссылками и с задержками,
            #сортируем на 2 списка.
            if 'http' in line:
                #Удаляем последний двойной символ новой строки 
                #(2 символа).
                line = line[:-1]
    ...
    def plan_one(self, num):
        ...
        Идея: сначала вызвать ф-ю plan(),потом передать num 
        и вызвать эту ф-ю.
        """
